...
export const FeatureSection = ({ data }: FeatureSectionProps) => {
  const getIcon = (idx: string) => {
    switch(idx) {
      case '01': return <Timer className="w-6 h-6" />;
      case '02': return <Smartphone className="w-6 h-6" />;
      case '03': return <Calculator className="w-6 h-6" />;
      default: return <BarChart3 className="w-6 h-6" />;
    }
  };

  return (
    <section id="about" className="w-full bg-transparent py-24 px-4 transition-colors">
